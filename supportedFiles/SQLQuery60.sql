use UOdata
insert into [UO] (EDRPOU, UOname, OKPO, UOfeature) select EDRPOU, UOname, OKPO, UOfeature from (select EDRPOU, UOname, OKPO, UOfeature from ( select QQ.[RECORD], EDRPOU, UOname, OKPO, UOfeature from (SELECT RECORD, EDRPOU, BENEFICIARY, LTRIM(LEFT([BENEFICIARY], CHARINDEX(';', [BENEFICIARY]) - 1)) AS UOname, '' AS OKPO, 'Бенефіціар' AS UOfeature
FROM     UOBeneficiares
UNION ALL
