select EDRPOU, UOname, OKPO, UOfeature into UO from ( select QQ.[RECORD], EDRPOU, UOname, OKPO, UOfeature from (SELECT RECORD, EDRPOU, BENEFICIARY, LTRIM(LEFT([BENEFICIARY], CHARINDEX(';', [BENEFICIARY]) - 1)) AS UOname, '' AS OKPO, 'Бенефіціар' AS UOfeature
FROM     UOBeneficiares
UNION ALL
...
FROM     UOfounders INNER JOIN
  ...
  group by  EDRPOU) AS QQ on Q.[EDRPOU] = QQ.[EDRPOU] and Q.REGISTRATION = QQ.REGISTRATION
				  ...
				  ) AS QQQ on QQ.RECORD = QQQ.RECORD) AS QQQQQ
